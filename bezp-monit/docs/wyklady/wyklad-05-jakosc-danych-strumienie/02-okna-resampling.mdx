...
- Dobrać odpowiedni rozmiar i typ okna czasowego (rectangular, Hanning, Hamming) dla różnych procesów (PV, wind, vibrations)
- Zastosować resampling i agregacje (mean, max, p95) do synchronizacji multi-rate data streams
- Zaprojektować kaskadę retencji (raw → 1-min → 15-min → hourly) z optymalizacją storage/query performance
- Ocenić trade-off między noise reduction a detection latency (window size vs. false positive rate)
</LearningObjective>

<SlideContainer>

<Slide title="🪟 Okna czasowe (Windowing) – wygładzanie sygnału" type="info">

<KeyPoints title="📋 Dlaczego windowing? Problem: Noise vs. Signal">

**Raw data** zawiera:
- **Signal** (real process: PV power, wind speed, temperature)
- **Noise** (measurement uncertainty, EMI, quantization, transients)

**Bez filtrowania**: Noise triggers false alarms (FP) → alarm fatigue  
**Z windowing**: Smooth signal → reduced FP, ale **trade-off**: Detection latency ↑
...
**1. Rectangular Window (prostokątny, "box average")**
...
Gdzie:
- N: Window size (liczba próbek)
- x(t): Raw measurement

**Charakterystyka**:
- **Uniform weights** (każda próbka równa waga 1/N)
- **Prostota** (easy to implement)
- **Wady**: Sharp cutoff → spectral leakage (ringing artifacts w frequency domain)
...
# Result: każdy punkt = average ostatnich 60 s
```
...
**Charakterystyka**:
- **Weights taper** (points na końcach okna mają mniejszą wagę → smooth transition)
...
**Różnica vs. Hanning**: Slightly higher sidelobes, but better mainlobe (trade-off)

**Application**: Similar do Hanning (spectral analysis), preferred w niektórych DSP contexts

---

**4. Exponential Weighted Moving Average (EWMA) – adaptive**
...
- α: Smoothing factor (0 < α < 1)
  - α → 1: More responsive (less smoothing)
  - α → 0: More smoothing (slower response)
...
**Przykład (pandas):**
```python
# EWMA dla PV power (α = 0.1 → heavy smoothing)
df['P_AC_ewma'] = df['P_AC'].ewm(alpha=0.1, adjust=False).mean()
```

</KeyConcept>

<SupportingDetails title="📊 Wybór rozmiaru okna: Process dynamics">

**Kluczowa zasada**: **Window size ∝ Process time constant**

### **Proces SLOW (PV power, temperature):**

- **Time constant**: τ ~ 1-10 min (clouds → irradiance change → power response)
- **Window size**: 1-5 min (60-300 s)
- **Rationale**: Noise high-frequency (1-10 Hz), signal low-frequency (&lt;0.1 Hz) → large window OK

**Przykład: PV power smoothing**
```python
# Raw: 1 Hz (1000 samples/min) → very noisy (inverter switching, measurement noise)
# Window: 60 s (60 samples) → smooth, preserves cloud transients (τ ~ 2-5 min)

df['P_AC_1min'] = df['P_AC'].rolling(window=60).mean()
```

**Impact:**
- **Noise reduction**: σ (std. dev) reduced by √N = √60 ≈ **7.7×** (Central Limit Theorem)
...
- **Time constant**: τ ~ 0.1-1 s (vibrations: bearing defects 10-100 Hz)
...
**Przykład: Wind speed (10 Hz sampling, gust detection)**
```python
# Raw: 10 Hz (100 samples/s) → dla KPI (10-min average)
df['Wind_10min'] = df['Wind'].rolling(window=6000).mean()  # 10 min × 60 s × 10 Hz
...
|-------------|-----------------|-------------------|---------------------|-------------|
| **Small** (10 s) | √10 ≈ 3× | +5 s (half window) | HIGH (noise passes) | Fast process, critical alarms |
| **Medium** (60 s) | √60 ≈ 8× | +30 s | MEDIUM | Balance (PV, most OZE) |
| **Large** (300 s, 5 min) | √300 ≈ 17× | +2.5 min | LOW | KPI, trending (nie alarms) |
...
<Example title="Case study: PV farm alarm – window size optimization">

**Problem**: Farma PV 10 MWp, alarm "Low PR" (PR < 75%) triggering **50× / day** (false positives, alarm fatigue).

**Root cause analysis:**
1. PR calculated every 1 s (real-time)
2. G_POA noise: σ = ±15 W/m² (±1.5% @ 1000 W/m²)
3. P_AC noise: σ = ±50 kW (±0.5% @ 10 MW)
4. Combined: PR noise σ ≈ ±2% → PR fluctuates 80% ± 2% → frequent dips &lt;75%

**Solution options:**

**Option A: Increase alarm threshold (75% → 70%)**
...
**Impact:**
- **Noise reduction**: √300 ≈ **17×** → σ_PR: ±2% → ±0.12% (poniżej threshold)
- **FP rate**: 50/day → **2/day** (-96%! Huge improvement)
...
- **Additional rule**: Alarm ONLY if PR &lt;75% persists for **2 consecutive windows** (10 min total) → further reduces transient FP
...
- Operator satisfaction: ↑↑ (alarm credibility restored)
...
**Przebieg**:
1. Dlaczego windowing (4 min) – noise vs. signal, false positives
2. Typy okien (5 min) – rectangular, Hanning, Hamming, EWMA (formulas + examples)
3. Wybór rozmiaru okna (4 min) – process dynamics (slow PV vs. fast vibrations), table trade-offs
4. Case study (4 min) – PV alarm optimization (96% FP reduction)
5. Q&A (2 min)

**Punkty kluczowe**:
- **"Window size = 3-5× process time constant"** rule of thumb
- **Noise reduction ∝ √N** (Central Limit Theorem) – duże okno = dramatyczna poprawa SNR
- **Trade-off**: Smoothing (FP reduction) vs. Latency (detection delay)
- **Separate windows** dla alarming (small) vs. KPI (large) – different objectives!

**Demonstracja praktyczna**:
- Live Python: Raw vs. smoothed signal (matplotlib plot)
- Comparison: 10 s vs. 60 s vs. 300 s window (pokazuje trade-off)
- Hanning vs. rectangular (frequency domain plot – pokazuje spectral leakage)
...
- "Bigger window = always better" – NO! Zależy od application (alarms vs. KPI)
- Applying large window do fast process (vibrations) → miss transients (defeats purpose)
- Nie rozróżnianie noise (measurement) od signal variability (real clouds) – window removes obie, ale to intended

**Pytania studenckie**:
- Q: Dlaczego Hanning zamiast rectangular?
- A: Dla **spectral analysis** (FFT): Hanning reduces spectral leakage (smooth taper → less ringing). Dla **simple averaging** (KPI): rectangular OK (prostszy, equivalent dla time-domain smoothing).

- Q: Jak dobrać α w EWMA?
- A: Empirically lub z process τ: α ≈ 2/(N+1), gdzie N = equivalent window size. For τ=60 s, α ≈ 0.03 (heavy smoothing). Tune based on desired responsiveness.
...
<Slide title="⏱️ Resampling i synchronizacja – multi-rate data streams" type="tip">

<KeyConcept title="Problem: Niejednorodne częstotliwości próbkowania">

**Real-world OZE system** ma **multiple data streams** z różnymi sampling rates:
...
**Challenge**: Jak analyze/correlate streams z różnymi rates? **Solution**: **Resampling** → common time grid.

</KeyConcept>

<SupportingDetails title="🔧 Resampling methods: Upsampling vs. Downsampling">

### **Downsampling (decimation): High rate → Low rate**

**Proces**: Reduce sampling rate (e.g., 1 Hz → 1/min)

**Method**: **Aggregation** (mean, max, min, sum, p95, ...)

**Example (PV power: 1 Hz → 1 min):**
```python
...
# Result: 60 samples/hour (60× reduction)
```
...
| **sum** | Totals (energy, fault counts, event counts) | $ S = \sum x_i $ | E_15min = sum(P_AC_1Hz) × Δt |
...
### **Upsampling (interpolation): Low rate → High rate**

**Proces**: Increase sampling rate (e.g., 1/min → 1 Hz)

**Method**: **Interpolation** (forward-fill, linear, polynomial, cubic spline)

**Example (temperature: 1/min → 1 Hz):**
```python
...
**Problem**: Streams z różnymi timestamps (jitter, clock skew, different sources).
...
# Now: All inverters on SAME timestamps → can sum/aggregate easily
df_farm = pd.concat([df_inv1_aligned['P_AC'], df_inv2_aligned['P_AC'], ...], axis=1).sum(axis=1)
```

**Tolerance**: Allow matching within ±500 ms (if no sample within tolerance → NaN).
...
**Analysis goal**: Correlate vibration spikes z power/RPM changes → diagnose fault.

**Step 1: Downsample vibrations (10 kHz → 1 Hz)**
```python
# Vibration raw: 10 kHz → very high volume (36 GB/hour!)
# Downsample: RMS (root-mean-square) every 1 s
df_vib_1Hz = df_vib_10kHz.resample('1S').apply(lambda x: np.sqrt(np.mean(x**2)))

# Result: 3600 samples/hour (10000× reduction, storage-friendly)
```
...
**Interpretation:**
- **Vibration vs. Power**: r = -0.68 (negative correlation!) → High vibration gdy LOW power
- **Vibration vs. RPM**: r = -0.72 (stronger negative) → High vibration @ LOW RPM
...
**Finding**: Vibration spikes @ 14:12, 14:28, 14:45 → correlate z RPM ramps (acceleration phase) → gearbox bearing fault confirmed.

**Action**: Scheduled maintenance (bearing replacement), prevented catastrophic failure (cost savings €200k).
...
**Przebieg**:
1. Multi-rate problem (3 min) – table różnych rates w OZE
2. Downsampling (4 min) – aggregation methods (mean, max, sum, p95)
3. Upsampling (2 min) – interpolation (forward-fill, linear)
4. Alignment (2 min) – common time grid, jitter handling
5. Case study (4 min) – wind turbine fault diagnosis, correlation

**Punkty kluczowe**:
- **Resampling = foundational** dla multi-source data analysis
- **Aggregation function matters** (mean ≠ max ≠ sum) – zależy od physical meaning
...
- Pokazuje different aggregations (mean vs. max) → different insights
- Alignment visualization (before/after: misaligned → aligned timestamps)

**Materiały pomocnicze**:
- Pandas time series documentation (`resample`, `reindex`, `asfreq`)
- Example notebook: Multi-rate OZE data processing
- NTP/PTP time sync best practices (link back to Wykład 02)

**Typowe błędy studenckie**:
- Upsampling thinking it adds resolution – NO, only interpolates (pretend)
- Nie specifying aggregation function (pandas default: mean, ale może nie be appropriate)
- Ignoring timezone i DST (resample @ DST transitions → gaps/overlaps)
...
- Q: Który aggregation dla energy billing (15-min intervals)?
- A: **sum(Power × Δt)** dla energy (Wh), lub **mean(Power) × interval_duration** (equivalent). Verify with meter reading (ground truth).
...
<Slide title="🗜️ Downsampling cascade i retencja – storage optimization" type="warning">
...
- **Data rate**: 500 points × 1 Hz × 8 bytes (float64) = **4 kB/s** = 345 MB/day = **126 GB/year** (raw!)

**For 20-year lifetime**: 126 GB × 20 = **2.5 TB** (per farm!) + compliance wymaga 10+ years retention.

**Solution**: **Downsampling cascade** + **Retention policies** (progressive aggregation + deletion)

</KeyConcept>

<SupportingDetails title="🎯 Retention cascade design – 4 tiers">
...
- **Storage**: 345 MB/day × 7 = **2.4 GB** (rolling window)

**Tier 2: 1-MINUTE aggregated**
- **Rate**: 1/min (60× reduction)
...
- **Storage**: 2.4 GB / 60 × 180 = **7.2 GB**

**Tier 3: 15-MINUTE aggregated**
- **Rate**: 1/15min (900× reduction vs. raw)
...
- **Storage**: 2.4 GB / 900 × 1095 days = **2.9 GB**
...
**Reduction**: 2500 GB → 12.6 GB = **200×** savings! 🎉

</SupportingDetails>

<Example title="InfluxDB retention policy – implementation">
...
-- Continuous Query (CQ): Auto-aggregate raw → 1-min every hour
...
**Automation**: Continuous Queries (CQ) run automatically (every hour) → progressive downsampling, old data auto-deleted per retention policy.
...
def query_power(start, end, farm_id):
    ...
    if duration <= 7 * 86400:  # ≤ 7 days → use RAW
        bucket = "raw_7d"
        interval = "1s"
    elif duration <= 180 * 86400:  # ≤ 6 months → use 1-MIN
        bucket = "aggregated_1min_6mo"
        interval = "1m"
    elif duration <= 1095 * 86400:  # ≤ 3 years → use 15-MIN
        bucket = "aggregated_15min_3y"
        interval = "15m"
    else:  # > 3 years → use HOURLY
...
**Optimization**: Automatic tier selection based on query range → fast queries (minimal data scanned).
...
| Aggregation | Interval | PR calculated | Δ vs. baseline | Storage (1 month) | Comment |
|-------------|----------|---------------|----------------|-------------------|---------|
| **Raw** | 1 Hz | 82.50% | 0.00% (reference) | 10.3 GB | Baseline, high storage |
| **1-MIN** | 60 s | 82.49% | -0.01% | 172 MB | Minimal error, 60× reduction ✅ |
| **15-MIN** | 900 s | 82.47% | -0.03% | 11.5 MB | Acceptable error, 900× reduction ✅ |
| **HOURLY** | 3600 s | 82.38% | -0.12% | 2.9 MB | Noticeable bias (smooths cloud events) ⚠️ |
| **DAILY** | 86400 s | 81.95% | -0.55% | 122 kB | Significant error (too coarse) ❌ |

**Findings:**
- **1-MIN aggregation**: Virtually identical to raw (&lt;0.01% error) → **RECOMMENDED minimum dla KPI**
- **15-MIN**: Standard dla grid telemetry (OSD compliance), error &lt;0.05% → **ACCEPTABLE dla billing/reporting**
- **HOURLY**: Smoothing introduces bias (-0.12%) → OK dla trending, **NOT for billing**
- **DAILY**: Too coarse (-0.55% error, misses intra-day variability) → **Archival only**
...
**Przebieg**:
1. Storage problem (3 min) – math shows 2.5 TB dla 20 years (impractical)
2. Retention cascade design (5 min) – 4 tiers (raw, 1-min, 15-min, hourly)
3. InfluxDB implementation (4 min) – retention policies + continuous queries
4. Trade-off analysis (3 min) – PR accuracy vs. aggregation interval
5. Q&A (2 min)

**Punkty kluczowe**:
- **Cascade downsampling** = mandatory (nie optional) dla long-term sustainability
- **200× storage reduction** (2.5 TB → 12 GB) bez significant loss accuracy
- **15-min standard** dla compliance (grid telemetry, billing)
- **Automation** (CQ / scheduled tasks) → no manual intervention
...
**Typowe błędy studenckie**:
- "Store everything raw forever" – impossible (storage/cost)
- Aggregating before preservation (irreversible) – cascade allows re-aggregation later
- Nie testing PR accuracy before deploying retention → discover error too late

**Pytania studenckie**:
- Q: Co jeśli discover error AFTER raw data deleted (>7 days)?
...
- Q: Czy można mieć różne retention dla różnych measurements (power vs. temps)?
- A: TAK! Separate retention policies per measurement type (power: 7 days raw, temps: 1 day raw – less critical). Flexible design.
...
<Slide title="📝 Quiz: Windowing, resampling, retention" type="info">
  ...
  questions={[
    {
      ...
      options: [
        "√60 ≈ 7.7× reduction in σ (std. dev)",
        "60× reduction (linear z window size)",
        "1/60 = 0.0167× (60× smaller)",
        "Zależy od noise type (nie można określić bez spektrum)"
      ],
      correctAnswer: 0,
      explanation: "Central Limit Theorem: σ_avg = σ_raw / √N. Dla N=60, reduction = √60 ≈ 7.7×. Opcja (b) nieprawda (linear only dla sum, nie average). Opcja (c) odwrotnie. Opcja (d) partially true (CLT assumes independent noise), ale √N to good approximation dla most cases."
    },
    {
      question: "False Positive rate: 50/day. Po zastosowaniu 5-min windowing: 2/day (-96%). Detection latency wzrósł o?",
      options: [
        "+2.5 min (half window, center=False)",
        "+5 min (full window)",
        "+10 min (2× window dla confirmation)",
        "Brak latency (windowing nie opóźnia)"
      ],
      correctAnswer: 0,
      explanation: "Windowing (center=False, default): Detection latency = half window size (signal musi 'fill' window). 5 min / 2 = +2.5 min. Opcja (b) full window tylko jeśli wymaga ENTIRE window above threshold (rare). Opcja (c) to additional confirmation rule (nie windowing itself). Opcja (d) nieprawda (windowing ALWAYS adds latency)."
    },
    {
      question: "Downsampling 1 Hz → 15 min: Która aggregacja dla G_POA (irradiancja) dla PR calculation?",
      options: [
      ...
      correctAnswer: 0,
      explanation: "PR = E_actual / E_expected, gdzie E_expected ∝ G_average (nie peak). Mean to correct aggregation (represents average energy available). Max/min to extremes (nie representative). Sum would need unit conversion (W/m² → Wh/m², requires × Δt)."
    },
    {
      question: "Retention cascade: raw 7d, 1-min 6mo, 15-min 3y. Query dla last 2 months KPI analysis – który tier?",
      options: [
      ...
      correctAnswer: 0,
      explanation: "2 months < 6 months → 1-min tier available i sufficient (0.01% error vs. raw, 60× faster query). Opcja (b) raw may nie być available (>7 days deleted) i overkill (slow query). Opcja (c) 15-min OK ale lower resolution (1-min better jeśli available). Opcja (d) hourly too coarse dla KPI."
    },
    {
      ...
      correctAnswer: 0,
      explanation: "Hourly aggregation smooths cloud events → G_average slightly higher (transients averaged) → E_expected overestimated → PR underestimated. Opcja (b) negligible (float64 precision >>0.1%). Opcja (c) nieprawda (monthly aggregation wouldn't be affected by single DST event). Opcja (d) irrelevant (drift affects raw too, nie aggregation bias)."
    }
  ]}
/>

:::tip Rekomendacja po quizie
Jeśli uzyskałeś &lt;80%, przejrzyj: (1) Noise reduction √N (CLT), (2) Aggregation functions (mean vs. max, context-dependent), (3) Retention tier selection (query optimization). Windowing i downsampling to core skills data engineeringu.
:::
...
**Windowing (wygładzanie)**:
   - **Noise reduction**: ∝ √N (60 s window → 7.7× reduction)
   ...
   - **Rule**: Window size = 3-5× process time constant

**Resampling (multi-rate synchronization)**:
   - **Downsampling**: Aggregation (mean, max, sum, p95) – context-dependent choice
   - **Upsampling**: Interpolation (forward-fill, linear) – synthetic data, flag accordingly
...
**Retention cascade (storage optimization)**:
   ...
   - **Storage savings**: 200× reduction (2.5 TB → 12 GB) bez significant accuracy loss
...
**Następne kroki:**
- Ćwiczenie: Implement retention cascade (InfluxDB config)
- Lab: Window size optimization (FP rate vs. latency trade-off)
- Przygotowanie: Następna sekcja – TSDB (Time-Series Databases) wybór i konfiguracja
