...
W poprzednich wykładach poznaliśmy **zagrożenia**, potem **systemy kontroli pitch i yaw** (wymowa: yaw → "jo" (like in Joga, but with a long "o")), a teraz widzimy, jak wygląda **pełna warstwa bezpieczeństwa**. Łączymy perspektywę **mechaniczną (hamulce)** i **systemową (bezpieczeństwo, procedury, redundancja)**.
</InstructorNotes>
...
Rysunek pokazuje szczegóły konstrukcji hamulca tarczowego umieszczonego na wale głównym turbiny, między generatorem a przekładnią. Widzimy tarczę hamulcową (typowa średnica 500-1500 mm w zależności od rozmiaru turbiny), zaciski hydrauliczne z wieloma tłokami, okładziny cierne o wysokiej pojemności termicznej oraz system mocowania do wału głównego. Konstrukcja zacisku wykorzystuje mechanizm sprężynowo-zamykany, hydraulicznie-otwierany (co to znaczy?), zapewniający działanie fail-safe – hamulec aktywuje się automatycznie przy utracie ciśnienia hydraulicznego, co zapewnia krytyczną ochronę bezpieczeństwa.

To jest rzeczywista konstrukcja stosowana w turbinach wiatrowych. Kluczowy element to **mechanizm fail-safe**: hamulec zamyka się sprężynowo, a otwiera się hydraulicznie. To oznacza, że gdy zanika ciśnienie hydrauliczne – z jakiegokolwiek powodu – hamulec automatycznie się zamyka. To nie jest opcja, ale **wymóg bezpieczeństwa**.
...
- Zużycie energii: 10 kWh na proces hamowania
...
Można to powiązać z konkretnymi przykładami: awaria pitch przy silnym wietrze to zagrożenie krytyczne, problemy z generatorem to zagrożenie wysokie, a powolna degradacja łożysk to zwykle zagrożenie średnie lub niskie. 
...
<SupportingDetails title="🛑 Redundancja w systemach hamulcowych">
**Hamulec główny (operacyjny):**
- Aerodynamiczny system pitch – podstawowy sposób zatrzymania wirnika, wspomagany hydraulicznymi hamulcami tarczowymi (postojowymi).

**Hamulec awaryjny:**
- Sprężynowe hamulce tarczowe fail-safe (hydrauliczne lub pneumatyczne) + mechaniczne blokady / sworznie blokujące wirnik lub układ yaw.

**Hamulec zapasowy / postojowy:**
- Elektromechaniczne hamulce sprzęgłowe w przekładni lub generatorze, używane do długotrwałego unieruchomienia.

**Zasilanie układów hamulcowych:**
- Sieć + UPS + baterie + awaryjny generator (tak, aby możliwe było zadziałanie hamulców nawet przy zaniku zasilania zewnętrznego).
</SupportingDetails>

<SupportingDetails title="🎛️ Redundancja w systemach kontrolnych">
**PLC główny:**
- Programowalny sterownik odpowiedzialny za normalne sterowanie turbiną.

**PLC bezpieczeństwa:**
- Dedykowany safety PLC (np. Siemens S7-1500F) realizujący funkcje awaryjne i nadzorujące.

**Hardware safety:**
- Wyłączniki bezpieczeństwa, przekaźniki bezpieczeństwa, twarde obwody E-STOP.

**Komunikacja:**
- Przemysłowy Ethernet (np. Profinet) + Modbus RTU / inne łącza szeregowe + GSM/LTE jako łącze zapasowe do systemu SCADA.
</SupportingDetails>

<SupportingDetails title="📊 Redundancja sensoryczna">
**Anemometry:**
- Min. 3 niezależne czujniki wiatru (np. dwa czaszowe + jeden ultradźwiękowy).

**Pitch sensors:**
- 3 czujniki położenia dla każdej łopatki (redundancja 2oo3).

**Temperature:**
- Wielopunktowe pomiary temperatury w kluczowych lokalizacjach (generator, przekładnia, łożyska, szafa sterownicza).

**Vibration:**
- Akcelerometry w różnych osiach i lokalizacjach (łożyska główne, przekładnia, góra wieży).
</SupportingDetails>
...
<SupportingDetails title="📊 Availability – formalnie">
**W klasycznym reliability engineering** (model wykładniczy, pojedynczy element z naprawami):
...
- **MTTR** – średni czas naprawy (Mean Time To Repair)

W praktyce farm wiatrowych często definiuje się availability czasową lub energetyczną (ile energii mogła wyprodukować vs ile faktycznie wyprodukowała przy uwzględnieniu przestojów technicznych).

**Przykład liczbowy:**
- MTBF = 1000 h, MTTR = 10 h
```
A = 1000 / (1000 + 10) ≈ 0,99
```
→ turbina jest dostępna ~99% czasu.
</SupportingDetails>

<Example title="Case study – Analiza redundancji w rzeczywistej turbinie">
**Turbina**: 3 MW, farma wiatrowa w północnej Polsce

**System hamulcowy – analiza redundancji**:

**Poziom 1: Pitch system (primary – aerodynamiczny)**
- MTBF: 8 760 godzin (~1 rok)
- MTTR: 72 godziny
- Availability: ≈ 99,2%

**Poziom 2: Mechanical brakes (secondary – hamulce tarczowe)**
- MTBF: 17 520 godzin (~2 lata)
- MTTR: 36 godzin
- Availability: ≈ 99,8%

**Poziom 3: Emergency brake / mechanical lock (backup)**
- MTBF: 35 040 godzin (~4 lata)
- MTTR: 36 godzin
- Availability: ≈ 99,9%

**Total System Availability** (uproszczone założenie):
- ≈ 99,99%
- odpowiada to ok. 52,6 min przestoju/rok

*Uwaga: przy takich trzech poziomach działających w logice „co najmniej jeden musi być sprawny" efektywna dostępność funkcji bezpiecznego zatrzymania jest praktycznie 100% (>> 99,99%) – warto zaznaczyć, że 99,99% traktujemy tu jako wartość uproszczoną dydaktycznie, a nie wynik dokładnego modelu.*

**Koszty vs. korzyści**:
- Koszt systemów redundantnych: +15% ceny turbiny
- Redukcja przestojów: ~80% mniej czasu offline
- Szacowany ROI: ~3,2 roku
</Example>

<InstructorNotes>

To jest **realny case study** z farmy w Polsce. **99,99% dostępności** oznacza ok. **52,6 min przestoju rocznie** – to bardzo dobry wynik jak na system hamulcowy turbiny 3MW. Warto podkreślić, że przy trzech poziomach redundancji działających w logice „co najmniej jeden musi być sprawny", efektywna dostępność jest praktycznie 100% (>> 99,99%). **Szacowany ROI ~3,2 roku** pokazuje, że redundancja się **zwraca** – koszt +15% ceny turbiny, ale redukcja przestojów ~80%.

Ten przykład łączy **dane techniczne** (Availability, MTBF, MTTR) z **danymi ekonomicznymi** (koszty przestojów, czas zwrotu inwestycji). Dzięki temu widać, że inżynier bezpieczeństwa musi myśleć nie tylko o normach i ryzyku, ale też o opłacalności rozwiązań, które proponuje. Wartość 99,99% traktujemy tu jako uproszczenie dydaktyczne, a nie wynik dokładnego modelu matematycznego.
</InstructorNotes>
...
**Najlepsze procedury niewiele dają, jeśli nikt ich nie ćwiczy**.
</InstructorNotes>
...
**Systemy bezpieczeństwa nie są dodatkiem do turbiny, ale jej fundamentem.** Każdy element projektowania, budowy i eksploatacji musi uwzględniać aspekty bezpieczeństwa od samego początku.
</KeyConcept>
